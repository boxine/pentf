...
/**
 * Convert a time to a human readable string like:
 * "1 day" or "2.5s"
...
function detailedStatus(config, state) {
    ...
    let str = `${label} at ${utils.localIso8601()}:\n\n`
    + `  ${done_count}/${tasks.length - skipped_count} done, ${running_count} running\n\n`;

    if (running_count > 0) {
        const now = performance.now();
        const formatted = running.map(t => {
            ...
            let out = `  ${t.name} ${color(config, timeColor, formatDuration(now - t.start))}`;
            if (t.resources.length) {
                out += `\n    ${color(config, 'gray', 'Resources:')} ${color(config, 'cyan', t.resources.join(', '))}`;
            }
